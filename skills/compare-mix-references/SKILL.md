...
7. 先把人工复核后的解释写回 `mix-reference-report.md`，再在 Codex 回复中完整呈现有证据的报告正文和图片，最后列绝对文件路径。没有 Stem 时，M05–M10 仍保存在输出目录，但在对话中合并说明为“证据不足”，不要连续内嵌六张空信息图。
...
7. 多参考 Median、IQR、Direction Consistency、Outlier 与来源可信度。只有一个参考时只展示原始差值；不得计算或表述 Robust Deviation、Outlier 或参考共性。
