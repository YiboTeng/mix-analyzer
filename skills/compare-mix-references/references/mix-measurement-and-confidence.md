...
- 1 个参考：只做一对一原始差值，不计算 Robust Deviation 或 Outlier Score，不称为“参考共性”或统计异常。
- 2 个参考：可显示范围，可信度 `中低`。
...
没有足够 Stem 时，`M05`–`M10` 仍可生成“证据不足”面板，清楚说明缺什么，不画零值假数据。文件中保留这些审计面板；对话正文不必逐张内嵌，可合并说明缺失证据并把重点留给有数据的图。
