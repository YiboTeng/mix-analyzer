...
def read_audio(path: Path, sample_rate: int = TARGET_SR) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    info = sf.info(path)
    if info.channels > 2:
        raise ValueError(f"仅支持 Mono/Stereo 音乐文件；{path} 是 {info.channels} 声道，禁止静默截断")
    if info.duration < 0.5:
        raise ValueError(f"音频过短，无法形成可靠分析：{path} ({info.duration:.3f}s)")
    audio, source_sr = sf.read(path, always_2d=True, dtype="float64")
    if not np.any(np.abs(audio) > 1e-8):
        raise ValueError(f"音频为静音或低于可分析电平：{path}")
