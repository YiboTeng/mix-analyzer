...
def load_project(path: Path) -> dict[str, Any]:
    ...
    resolved_mix_paths = [str(Path(item["mix"]).resolve()).casefold() for item in items]
    if len(resolved_mix_paths) != len(set(resolved_mix_paths)):
        raise ValueError("Target 与 Reference 不能指向同一个 Mix 文件，Reference 之间也不得重复")
    return payload
...
def reference_features(records: dict[str, Any]) -> tuple[list[str], dict[str, list[float]]]:
    ...
    for label, record in records.items():
        values[label] = [
            ...
            float(np.mean(record["stereo"]["band_side_mid_db"][7:10])),
...
def reference_summary(records: dict[str, Any]) -> dict[str, Any]:
    ...
    raw_delta = target - median
    if reference_count == 1:
        # 单个样本没有离散度，不能用固定兜底分母伪造稳健 z-score。
        robust_deviation = [None] * len(names)
        ...
        robust_deviation = ((target - median) / scale).tolist()
        outliers = {
            label: float(np.median(np.abs((row - median) / scale)))
            for row, label in zip(references, labels[1:])
        }
        statistical_status = "preliminary_range" if reference_count == 2 else "robust_reference_interval"
    feature_summary = {
        name: {
            "target": target[index], "reference_median": median[index],
            "reference_q1": q1[index], "reference_q3": q3[index],
            "target_minus_reference_median": raw_delta[index],
            "robust_deviation": robust_deviation[index],
...
def plot_reference_set(records: dict[str, Any], summary: dict[str, Any], out_dir: Path) -> None:
    ...
    deviations = [summary["features"][name].get("robust_deviation") for name in names]
    single_reference = all(value is None for value in deviations)
    values = [summary["features"][name]["target_minus_reference_median"] for name in names] if single_reference else deviations
    axes[0].bar(x, values, color=PALETTE[0])
    axes[0].axhline(0, color="#222", lw=1)
    axes[0].set_xticks(x, names, rotation=35, ha="right")
    left_title = "Target − single reference (raw delta)" if single_reference else "Target robust deviation"
    left_limit = "单参考只显示原始差值，不是统计异常" if single_reference else limitation
    axes[0].set_title(confidence_title(left_title, confidence, left_limit))
    style_axes(axes[0])
    if summary.get("outliers"):
        outlier_labels = list(summary["outliers"])
        ref_x = np.arange(len(outlier_labels))
        axes[1].bar(ref_x, [summary["outliers"][label] for label in outlier_labels], color=PALETTE[1:1+len(outlier_labels)])
        axes[1].set_xticks(ref_x, outlier_labels, rotation=15)
...
def write_report(project: dict[str, Any], records: dict[str, Any], summary: dict[str, Any],
    ...
    comparisons = [
        ...
        ("High Side/Mid dB", float(np.mean(target["stereo"]["band_side_mid_db"][7:10])), median_reference(records, lambda r: float(np.mean(r["stereo"]["band_side_mid_db"][7:10]))), "高"),
    ...
    reference_charts = [
        ("M11_reference_intervals.png", "单参考原始差值或多参考稳健偏差"),
        ("M12_reference_matching_confidence.png", "参考匹配与来源可信度"),
    ]
    for filename, title in core_charts:
        lines += [f"### {title}", "", f"![{title}]({filename})", ""]
    has_stem_evidence = any(record.get("stems") for record in records.values())
    if has_stem_evidence:
        for filename, title in stem_charts:
            lines += [f"### {title}", "", f"![{title}]({filename})", ""]
    else:
        lines += [
            "### Stem 维度（证据不足）", "",
            "本次没有可靠 Stem，因此 M05–M10 只作为审计面板保存在输出目录，不在正文连续嵌入；它们不支持人声/伴奏、冲突、鼓、Bass、Lead 或 Pad 的单轨结论。", "",
        ]
        lines += [f"- `{filename}`：{title}" for filename, title in stem_charts]
        lines.append("")
    for filename, title in reference_charts:
        lines += [f"### {title}", "", f"![{title}]({filename})", ""]
    lines += ["## 最高优先级调整", ""]
